...
con = dbConnect(SQLite(),dbname = "many-dax-1-25-15/participants.db");
...
df.complete$currentVersion1 = str_detect(df.complete$beginhit, "2015-01-25")
df.complete$currentVersion2 = str_detect(df.complete$beginhit, "2015-01-24")
...
#Run 3, new within-subjects version plus manipulation check, 1/24-25/15
df.complete = df.complete[df.complete$currentVersion1 == TRUE | df.complete$currentVersion2 == TRUE,]
#NOTE! This includes 1 or 2 people who managed to complete the broken version on morning of 1/24, but they should be
#filtered out by the data structuring below since they don't have the right kinds of questions.
...
#New data cleaning!  Drop people who didn't answer the manipulation check questions correctly
...
###Mike code from last time
##Mike: Response variable histograms. RTs go a little long, maybe clip these at 8s? 
...
##Mike: Basic summary. This is the first finding, that N/V construal changes the weighting on manner/path generalization. I think this is kinda cool, actually. 
