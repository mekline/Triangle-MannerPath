...
[x, y, lens] = getPath('to'); %x and y are the top lh corner of the object, lens is number of (motion) frames, 30 = 1 sec
[x, y] = smoothPath(x,y); %Ensures that points are equidistant along that piecewise path...
 
[x, y, rotations] = applyManner('sineWave', x,y);
...
for obj = 1:1
    ...
    if strcmp(mode,'movie')
        ...
        withbridge = drawOnBackground(bridgeimg, backimg, 1, 1);
        ...
        for i = 1:lens
            ...
            %corner correctly!
            [t u v] = size(img_rot);
            %draw triangle & bridge on background
            withbridge = drawOnBackground(bridgeimg, backimg, 1, 1);
